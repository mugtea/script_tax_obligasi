...
DELETE FROM ReportParameter WHERE Fk_MsStatementTemplate_Id IN (select Pk_MsStatementTemplate_Id from MsStatementTemplate where [StatementTemplateName] IN ('Tax Reporting Casa TD'))

delete from StatementGeneratorPrimaryKey where FK_StatementGenerator_ID in (select PK_StatementGenerator_ID From StatementGenerator where StatementGeneratorName in ('Tax Reporting Casa TD') )

DELETE [dbo].[StatementGeneratorStatementParameterMapping] WHERE FK_StatementGenerator_ID IN (SELECT PK_StatementGenerator_ID fROM StatementGenerator WHERE StatementGeneratorName IN ('Tax Reporting Casa TD') )

DELETE FROM STATEMENTGENERATOR WHERE StatementGeneratorName IN ('Tax Reporting Casa TD')

DELETE FROM MSSTATEMENTTEMPLATE WHERE STATEMENTTEMPLATENAME in ( 'Tax Reporting Casa TD')

--sesuaikan path ssrs nya
INSERT INTO [dbo].[MsStatementTemplate] ( [StatementTemplateName], [ReportPath], [ReportName], [Activation], [CreatedDate], [CreatedBy], [LastUpdatedDate], [LastUpdatedBy], [ApprovedDate], [ApprovedBy], [Description], [EMateraiTemplateID], [JarakAtasEMaterai], [JarakKiriEMaterai]) VALUES ('Tax Reporting Casa TD', 'Danamon/TaxReports', 'TaxCustomerDeductedAdvice_Obligasi_Detail', '1', GETDATE(), 'system', GETDATE(), 'system', GETDATE(), 'system', '', (select MAX(Pk_MsStatementTemplate_Id)+1 from MsStatementTemplate), 0, 0);
  
DELETE FROM [SystemParameter] WHERE ParameterName IN ('EngineTaxStatus','Fk_MsStatementTemplate_Id_Tax_CasaTD','UrlService_TaxReporting')
...
DELETE FROM StatementGeneratorEmailTemplate WHERE FK_StatementGenerator_ID NOT IN (select PK_StatementGenerator_ID from StatementGenerator) or FK_StatementGenerator_ID in (select PK_StatementGenerator_ID from StatementGenerator where StatementGeneratorName in ('Tax Reporting CASA TD'))
...
'', --[EmailCC],
'', --[EmailBCC],
'Laporan Konsolidasi Bukti Potong Pajak', --[EmailSubject],
'<p>Nasabah Yang Terhormat,<br /><br />Terlampir adalah  Laporan Konsolidasi Bukti Potong Pajak Rekening  Bank Danamon sesuai permintaan Anda<br />Terima kasih,</p>  <p>PT Bank Danamon Indonesia Tbk</p>', --[EmailBody],
...
DELETE From DatabaseFriendlyField where FK_DatabaseFriendly_ID in (select PK_DatabaseFriendly_ID From DatabaseFriendly where OriginalTable in ('CustomerTaxDeductedAdvice_OBS_CASATD_CustomerInformation'))

DELETE From DatabaseFriendly where OriginalTable in ('CustomerTaxDeductedAdvice_OBS_CASATD_CustomerInformation')
